            ...
            else if (NetworkHelper.Instance.ConnectionInformation.IsInternetAvailable)
            {
                try
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(DownloadPath))
                    {
                        using (Stream DownloadStream = await DownloadBingPictureAsync(DownloadPath))
                        {
                            if (await FileSystemStorageItemBase.CreateNewAsync(PicturePath, CreateType.File, CollisionOptions.OverrideOnCollision) is FileSystemStorageFile PictureFile)
                            {
