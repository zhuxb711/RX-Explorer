    ...
    public sealed class FilterController : INotifyPropertyChanged
    {
        ...
        public List<FileSystemStorageItemBase> GetFilterCollection()
        {
            ...
            if (NameCondition != NameFilterCondition.None)
            {
                ...
                {
                    NameFilterResult.AddRange(OriginCopy.Where((Item) => (Item.Name.FirstOrDefault() >= 32 && Item.Name.FirstOrDefault() < 65) || (Item.Name.FirstOrDefault() > 90 && Item.Name.FirstOrDefault() < 97) || (Item.Name.FirstOrDefault() > 122 && Item.Name.FirstOrDefault() <= 126)));
                }
            }
