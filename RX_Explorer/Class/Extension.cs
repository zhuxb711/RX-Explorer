    ...
    public static class Extension
    {
        ...
        public static async Task SetStorageItemDataAsync(this DataPackage Package, params FileSystemStorageItemBase[] Collection)
        {
            try
            {
                ...
                IEnumerable<(string Path, IStorageItem CoreItem)> CoreStorageItemTupleList = await Task.WhenAll(NormalItems.OfType<ICoreFileSystemStorageItem<IStorageItem>>().Select((Item) => Item.GetStorageItemAsync().AsTask().ContinueWith((Previous) => (Item.Path, Previous.Result), TaskContinuationOptions.ExecuteSynchronously)));
