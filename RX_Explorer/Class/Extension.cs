    ...
    public static class Extension
    {
        ...
        public static async Task SetStorageItemDataAsync(this DataPackage Package, params FileSystemStorageItemBase[] Collection)
        {
            try
            {
                ...
                IEnumerable<IStorageItem> CoreStorageItemList = (await Task.WhenAll(NormalItems.OfType<FileSystemStorageFile>()
                                                                                               .Select((Item) => Item.GetStorageItemAsync().ContinueWith<IStorageItem>((PreviousTask) => PreviousTask.Result, TaskContinuationOptions.ExecuteSynchronously))
                                                                                               .Concat(NormalItems.OfType<FileSystemStorageFolder>()
                                                                                                                  .Select((Item) => Item.GetStorageItemAsync().ContinueWith<IStorageItem>((PreviousTask) => PreviousTask.Result, TaskContinuationOptions.ExecuteSynchronously))))).OfType<IStorageItem>();
