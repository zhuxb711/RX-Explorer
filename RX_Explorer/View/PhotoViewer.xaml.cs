        ...
        private async void Adjust_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (PhotoGirdView.SelectedItem is PhotoDisplayItem Item)
                {
                    ...
                    {
                        Frame.Navigate(typeof(PhotoEditor), Item, new DrillInNavigationTransitionInfo());
                    }
                    else
                    {
                        Frame.Navigate(typeof(PhotoEditor), Item, new SuppressNavigationTransitionInfo());
                    }
                }
            }
            catch (Exception ex)
            {
                LogTracer.Log(ex, $"An exception was threw when navigating to {nameof(PhotoEditor)}");
            }
        }
                        ...
                        using (Stream PhotoStream = await Item.PhotoFile.GetStreamFromFileAsync(AccessMode.Read, OptimizeOption.Sequential))
                        {
                            ...
                            using (Stream TempStream = await FileSystemStorageItemBase.CreateTemporaryFileStreamAsync(TempFilePath, IOPreference.PreferUseMoreMemory))
                            {
                                ...
                                else
                                {
                                    ...
                                    if (await FileSystemStorageItemBase.CreateNewAsync(Path.Combine(ApplicationData.Current.TemporaryFolder.Path, $"{Guid.NewGuid():N}{Item.PhotoFile.Type.ToLower()}"), CreateType.File, CollisionOptions.RenameOnCollision) is FileSystemStorageFile TempFile)
                                    {
                                        try
                                        {
