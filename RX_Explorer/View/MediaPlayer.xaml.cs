        ...
        private async void Binder_Binding(MediaBinder sender, MediaBindingEventArgs args)
        {
            ...
            try
            {
                if (await FileSystemStorageItemBase.OpenAsync(args.MediaBinder.Token) is FileSystemStorageFile MediaFile)
                {
                    ...
                    {
                        SLEInputStream MediaStream = new SLEInputStream(await MediaFile.GetStreamFromFileAsync(AccessMode.Read, OptimizeOption.RandomAccess), SecureArea.AESKey);
                        ...
                        if (MediaStream.Header.Core.Version >= SLEVersion.SLE150)
                        {
                            ...
                            try
                            {
                                switch (Path.GetExtension(MediaStream.Header.Core.FileName).ToLower())
                                {
                                    ...
                                    case ".m4a":
                                        {
                                            using (SLEInputStream MediaInfoStream = new SLEInputStream(await MediaFile.GetStreamFromFileAsync(AccessMode.Read, OptimizeOption.RandomAccess), SecureArea.AESKey))
                                            {
